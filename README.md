...
The Babylon greybox now spans a 28 × 52 room-based maze—just over three times its former floor area—with variable wall openings. Ten dangerous dragons are randomly distributed through its rooms on each new load; all ten can initiate attacks and remain valid spell targets, while only the nearest eligible dragon attacks at one time. Twelve berry bushes, eight inset fountains, four geodes, and eight potions spread rewards across the route. Two found runes open the inner door, all four found runes open the final Moon Door, and crossing that portal removes the Witch to complete the route. Add `?mazeSeed=your-seed` to reproduce a particular wall and dragon arrangement.
